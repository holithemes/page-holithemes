...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createShapes() {
      ...
      shapes.forEach((shape, index) => {
        ...
        // Get color for glow based on shape type
        let glowColor;
        if (shape.type === 'hexagon') glowColor = '#ec4899';
        else if (shape.type === 'circuit') glowColor = '#6366f1';
        else if (shape.type === 'diamond') glowColor = '#06b6d4';
        else glowColor = '#10b981';
        
        // Add pulsing animation to dot
        dot.style.animation = 'pulse-glow 2s infinite ease-in-out';
        dot.style.boxShadow = `0 0 5px 2px ${glowColor}`;
        ...
        shapeEl.addEventListener('mouseenter', () => {
          shapeEl.style.boxShadow = `0 0 10px 2px ${glowColor}, 0 0 20px 5px ${glowColor}`;
        });
